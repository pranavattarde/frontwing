...
export function TelemetryCard({
  ...
  useEffect(() => {
    ...
    const drawChannel = (met, yOffset, channelHeight, isMultiSubplot = false) => {
          ...
          pts.forEach((pt) => {
            ...
            if (bVal > 10) {
              ...
              if (inBrakeZone) {
                ...
                ctx.lineTo(x, getY(0));
                ctx.closePath();
                ctx.fillStyle = isDriverA ? "rgba(225, 6, 0, 0.15)" : "rgba(255, 214, 0, 0.1)";
                ctx.fill();
                ctx.beginPath();
              }
            }
      ...
      // Render Driver B (Defender - #FFD600) then Driver A (Chaser - Speed Red / driver color)
      if (dataB.length > 0) {
        renderTrace(dataB, driverB?.color || "#FFD600", false);
      }
      if (dataA.length > 0) {
        renderTrace(dataA, driverA?.color || (met === "brake" ? "#E10600" : "#E10600"), true);
      }
    };
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "bg-surface-base border border-border-subtle rounded p-4 flex flex-col justify-between select-none relative transition-all duration-fast",
        className
      )}
    ...
    >
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 border-b border-border-subtle pb-2.5 font-mono text-xs">
        ...
        {!isCollapsed && (
          <div className="flex items-center gap-1 bg-surface-raised border border-border-subtle rounded p-0.5">
        ...
        >
          ...
          {activeHoverDist !== null && !isCollapsed && (
            <>
              ...
              <div
                className="absolute top-0 bottom-0 pointer-events-none border-l border-dashed border-text-muted/60"
                style={{
                  left: `${(activeHoverDist / totalDistance) * (dimensions.width - 50) + 40}px`
                }}
              />

              {/* HUD Tooltip */}
              <div
                className="absolute top-2 pointer-events-none bg-surface-elevated/95 backdrop-blur-md border border-border-strong rounded p-2 text-xs font-mono shadow-xl z-20 flex flex-col gap-1 min-w-[150px]"
                style={{
                  left: Math.min(
                    dimensions.width - 170,
                    Math.max(10, (activeHoverDist / totalDistance) * (dimensions.width - 50) - 40)
                  )
                }}
              >
                ...
                <div className="flex flex-col gap-1 text-[11px]">
                  ...
                  {driverB?.code && ptB && (
                    <div className="flex justify-between items-center font-bold" style={{ color: driverB?.color || "#FFD600" }}>
      ...
      {activeMetric === "multi" && !isCollapsed && hasData && (
        <div className="mt-3 p-3 bg-surface-base border border-border-subtle rounded flex flex-col gap-2 font-mono text-xs">
            ...
            <div className="flex items-center gap-3">
              ...
              {driverB?.code && (
                <span className="flex items-center gap-1.5 text-timing-yellow font-bold">
                  <span className="w-2.5 h-0.5 bg-timing-yellow rounded-full inline-block" />
                  {driverB.code}
                </span>
              )}
            </div>
          ...
          <p className="text-[11px] text-text-muted border-t border-border-subtle pt-2 italic">
                ...
                {dataA.slice(0, 30).map((ptAItem, idx) => {
                  ...
                  return (
                    <tr
                      key={idx}
                      className={cn(
                        "hover:bg-surface-raised transition-colors type-tabular",
                        activeHoverDist !== null && Math.abs(dist - activeHoverDist) < 50
                          ? "bg-accent-primary/15 font-bold"
                          : ""
                      )}
                    >
                      <td className="py-1 px-2.5 text-text-primary">{dist.toFixed(1)}</td>
                      <td className="py-1 px-2 font-bold" style={{ color: driverA?.color || "#E10600" }}>
                        {spdA.toFixed(0)}
                      </td>
                      {driverB?.code && (
                        <td className="py-1 px-2 font-bold" style={{ color: driverB?.color || "#FFD600" }}>
                      ...
                      {driverB?.code && (
                        <td
                          className={cn(
                            "py-1 px-2 font-bold",
                            deltaSpd >= 0 ? "text-timing-green" : "text-accent-danger"
                          )}
                        >
                      ...
                      <td className="py-1 px-2">{brkA.toFixed(0)}%</td>
                      {driverB?.code && (
                        <td className="py-1 px-2">{ptBItem ? `${brkB.toFixed(0)}%` : "-"}</td>
                      )}
                    </tr>
                  );
                })}
