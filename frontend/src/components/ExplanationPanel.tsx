...
export function ExplanationPanel({ steps, conclusion, className }: ExplanationPanelProps) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className={cn('border border-fw-border rounded-card bg-panel overflow-hidden', className)}>
      {/* Header Toggle */}
      <button
      ...
      >
        ...
        <span className="text-mono-meta font-mono text-drs-cyan hover:underline">
          {isOpen ? '[HIDE_CHAIN]' : '[EXPLAIN_REASONING]'}
        </span>
      </button>
        ...
        {isOpen && (
          ...
          >
            <div className="p-4 flex flex-col gap-4">
              {/* Conclusion block */}
              <div className="bg-canvas border border-fw-border p-3 rounded-card text-sm text-text-primary border-l-2 border-l-teammate-yellow leading-relaxed">
              ...
              <div className="flex flex-col gap-3 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-[1px] before:bg-fw-border">
                {steps.map((step, idx) => (
                  <div key={idx} className="flex gap-4 relative pl-6">
                    {/* Bullet indicator */}
                    <div className="absolute left-[5px] top-1.5 w-1.5 h-1.5 rounded-full bg-teammate-yellow border border-canvas" />
