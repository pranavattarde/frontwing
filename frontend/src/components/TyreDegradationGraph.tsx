...
export function TyreDegradationGraph({
  ...
  const getYWear = (pct: number) => height - padding - (pct / 100) * (height - 2 * padding);
    ...
    <div className={cn('bg-panel border border-fw-border rounded-card p-4 flex flex-col gap-3', className)}>
      <div className="flex justify-between items-center text-mono-meta font-mono">
        ...
        <div className="flex items-center gap-3 text-[10px]">
          <span className="text-text-muted">{driverCode.toUpperCase()} COMPOUND: <span className="text-amber-400 font-bold">{latest?.compound || 'MEDIUM'}</span></span>
          <span className="text-text-muted">LIFE: <span className="text-drs-cyan font-bold">{latest?.wear_pct || 75}%</span></span>
        </div>
      </div>
      ...
      <div className="relative w-full h-[160px]">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
          ...
          <path d={wearPath} fill="none" stroke="#F59E0B" strokeWidth="2.5" strokeLinecap="round" />
