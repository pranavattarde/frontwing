...
import type { Stint, BreadcrumbItem } from '@/lib/types';
...
export function StrategyPlayground() {
  ...
  // Dynamic simulation simulation configurations based on pit lap
  const simData = useMemo(() => {
    ...
    if (pitLap === 22) {
      return {
        ...
        result: {
          ...
          delta: { positions: 0, seconds: 0.0 },
          confidence: 99,
          simType: 'v1_single' as const,
        },
        rivals: [
    ...
    // LAP 23-30 (Late Stop)
    const lateStints: Stint[] = [
      { compound: 'medium', startLap: 1, endLap: pitLap, wearSlope: 0.078, isActual: false },
      { compound: 'hard', startLap: pitLap + 1, endLap: 53, wearSlope: 0.054, isActual: false },
      { compound: 'medium', startLap: 54, endLap: 71, wearSlope: 0.070, isActual: false },
    ];
    return {
      stints: lateStints,
      result: {
  ...
  const sliderMarkers = [
    { value: 18, label: 'EARLY_TRAFFIC', type: 'default' as const },
    { value: 20, label: 'OPTIMAL_P2', type: 'optimal' as const },
    { value: 22, label: 'ACTUAL_P3', type: 'actual' as const },
    { value: 24, label: 'LATE_LOSS', type: 'default' as const },
  ];

  const handleSliderChange = (val: number) => {
    setIsComputing(true);
    // Mimic simulation pipeline loading
    setTimeout(() => {
      setPitLap(val);
    ...
    <div className="min-h-screen bg-canvas text-text-secondary flex flex-col font-sans selection:bg-drs-cyan/20 selection:text-drs-cyan">
      ...
      <main className="flex-1 w-full max-w-[1440px] mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="border-b border-fw-border pb-4">
          ...
          <section className="bg-panel border border-fw-border rounded-card p-4 border-l-2 border-l-drs-cyan">
            <span className="text-mono-meta font-mono text-text-muted uppercase tracking-wider block mb-1">
        ...
        <div className="flex flex-col gap-6">
          ...
          <section className="flex flex-col gap-3">
            ...
            <PitWindowVisualizer
              pittingDriver={{ code: 'SAI', exitLap: pitLap, pitLossTime: 22.0 }}
