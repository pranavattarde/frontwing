...
export function LapTimeGraph({
  ...
  return <div className={cn("bg-panel border border-fw-border rounded-card p-4 flex flex-col gap-3", className)}><div className="flex justify-between items-center text-mono-meta font-mono"><span className="text-text-primary font-semibold tracking-wider flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-drs-cyan" />
        ...
        </span><div className="flex items-center gap-4 text-[10px]"><span className="text-drs-cyan font-semibold">{driverCode}</span>{comparativeDriverCode && <span className="text-f1-red font-semibold">{comparativeDriverCode}</span>}</div></div><div className="relative w-full h-[180px]"><svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">{[0, 0.25, 0.5, 0.75, 1].map((pct, idx) => {
    const yVal = minTime + (maxTime - minTime) * (1 - pct);
    const yPos = getY(yVal);
    return <g key={idx}><line x1={padding} y1={yPos} x2={width - padding} y2={yPos} stroke="#1C2025" strokeDasharray="3 3" /><text x={padding - 6} y={yPos + 3} textAnchor="end" fill="#5C6470" fontSize="9" className="font-mono">{yVal.toFixed(1)}s
                </text></g>;
  })}<path d={pathA} fill="none" stroke="#00E5FF" strokeWidth="2.5" strokeLinecap="round" />{pathB && <path d={pathB} fill="none" stroke="#FF1801" strokeWidth="2" strokeDasharray="4 2" />}{data.map((d, i) => <circle
    key={i}
    cx={getX(d.lap)}
    cy={getY(d.lap_time)}
  ...
  ><div>LAP {hoveredLap.lap}: <span className="text-drs-cyan font-bold">{hoveredLap.lap_time.toFixed(3)}s</span></div>{hoveredLap.compound && <div className="text-text-muted">COMPOUND: {hoveredLap.compound}</div>}</div>}</div></div>;
}
