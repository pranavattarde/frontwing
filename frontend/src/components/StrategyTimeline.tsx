...
interface StrategyTimelineProps {
  ...
  driverCode: string;
  simulated?: Stint[];
  variant?: 'single' | 'comparison' | 'grid';
  onStintClick?: (stintIndex: number, isSimulated: boolean) => void;
}
...
export function StrategyTimeline({
  ...
  const renderStintRow = (rowStints: Stint[], isSim: boolean) => {
    return (
      <div className="relative h-6 w-full flex items-center bg-elevated/40 border border-fw-border rounded-sm overflow-hidden select-none">
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:flex flex-col items-center z-50">
                <div className="bg-panel border border-fw-border-active rounded-card p-2 text-text-primary text-[10px] leading-tight shadow-lg pointer-events-none whitespace-nowrap">
                  <p className="font-semibold uppercase text-drs-cyan">{stint.compound} COMPOUND</p>
                  <p>Laps: {stint.startLap} - {stint.endLap} ({stintLaps} laps)</p>
                  <p>Degradation: {stint.wearSlope.toFixed(3)} s/lap</p>
                </div>
                <div className="w-1.5 h-1.5 bg-panel border-r border-b border-fw-border-active rotate-45 -mt-1" />
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="w-full flex flex-col gap-2 p-2 bg-panel rounded-card border border-fw-border">
      {/* Timeline Header */}
      ...
      <div className="flex flex-col gap-1.5 w-full">
        ...
        <div className="flex items-center gap-2">
          {isComparison && (
            <span className="w-10 text-[10px] font-mono text-text-muted text-right shrink-0">ACTUAL</span>
          )}
        ...
        {isComparison && simulated && (
          <div className="flex items-center gap-2">
            <span className="w-10 text-[10px] font-mono text-drs-cyan text-right shrink-0">SIM_L20</span>
      ...
      {/* Axis Guide */}
      <div className="relative w-full h-4 mt-1 font-mono text-[9px] text-text-muted select-none">
