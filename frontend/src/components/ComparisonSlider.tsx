...
export function ComparisonSlider({
  ...
  return (
    <div className={cn('flex flex-col gap-3 p-3 bg-panel border border-fw-border rounded-card select-none w-full', className)}>
      <div className="flex justify-between items-center text-mono-meta font-mono">
        ...
        <div className="flex items-center gap-2">
          {isComputing && (
            <span className="text-teammate-yellow animate-pulse text-[10px]">
              RE_COMPUTING...
            </span>
          )}
          <span className="text-drs-cyan text-xs font-semibold">LAP {internalVal}</span>
        </div>
      </div>
      ...
      {/* Slider Markers */}
      {markers.length > 0 && (
        <div className="relative h-6 mt-1.5 font-mono text-[9px] text-text-muted">
          {markers.map((m, idx) => {
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isMatch
                      ? 'bg-drs-cyan border-drs-cyan scale-125'
                      : m.type === 'optimal'
                      ? 'bg-tire-inter border-tire-inter/50'
                      : m.type === 'actual'
                      ? 'bg-f1-red border-f1-red/50'
                      : 'bg-elevated border-fw-border'
                  )}
                />
                <span
                  className={cn(
