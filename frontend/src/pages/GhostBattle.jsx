...
export function GhostBattle() {
  ...
  const [lapNumber, setLapNumber] = useState(42);
  const breadcrumbs = [
    { label: "Home", href: "/" },
  ...
  />{
    ...
    /* Lap Selector */
  }<div className="flex items-center gap-3 bg-panel border border-fw-border rounded-card p-2"><span className="text-mono-meta font-mono text-text-muted uppercase">ACTIVE_LAP:</span><div className="flex gap-1">{[40, 41, 42, 43, 44].map((lap) => <button
    key={lap}
    onClick={() => setLapNumber(lap)}
    className={`px-2 py-1 font-mono text-xs rounded-button border transition-all ${lapNumber === lap ? "border-drs-cyan text-drs-cyan bg-drs-cyan/5 font-semibold" : "border-fw-border text-text-muted hover:text-text-secondary"}`}
  >
                  L{lap}</button>)}</div></div></div>{
