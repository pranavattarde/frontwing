...
export function SectorComparisonGraph({
  ...
  return <div className={cn("bg-panel border border-fw-border rounded-card p-4 flex flex-col gap-4", className)}><div className="flex justify-between items-center text-mono-meta font-mono"><span className="text-text-primary font-semibold tracking-wider flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-drs-cyan" />
    ...
    ><div className="flex justify-between items-center text-mono-meta font-mono mb-1"><span className="font-bold text-text-primary">{s.sector}</span><span className={cn("font-mono text-xs font-bold", isFaster ? "text-drs-cyan" : "text-f1-red")}>{deltaStr}</span></div><div className="flex justify-between items-center text-[10px] font-mono text-text-muted"><span>TIME: {s.driver_time.toFixed(3)}s</span><span>REF: {s.benchmark_time.toFixed(3)}s</span></div></div>;
  })}</div></div>;
}
