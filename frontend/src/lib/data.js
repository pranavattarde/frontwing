export const AUSTRIAN_GP = {
  ...
  date: "2024-06-30",
  totalLaps: 71,
  weather: "Dry, 28\xB0C air, 48\xB0C track"
};
...
export const RACE_RESULTS = [
  {
    driver: DRIVERS.RUS,
    scores: { composite: 85.5, strategy: 88, tire: 82, pace: 86, pit: 90, execution: 82 },
    position: 1,
    gridPosition: 3,
    status: "Classified"
  },
  {
    driver: DRIVERS.PIA,
    scores: { composite: 88.7, strategy: 90, tire: 91, pace: 89, pit: 85, execution: 88 },
    position: 2,
    gridPosition: 5,
    status: "Classified"
  },
  ...
  {
    driver: DRIVERS.HAM,
    scores: { composite: 80.1, strategy: 82, tire: 76, pace: 78, pit: 86, execution: 79 },
    position: 4,
    gridPosition: 6,
    status: "Classified"
  },
  {
    driver: DRIVERS.VER,
    scores: { composite: 74.8, strategy: 68, tire: 72, pace: 92, pit: 80, execution: 62 },
    position: 5,
    gridPosition: 1,
    status: "Classified"
  },
  {
    driver: DRIVERS.NOR,
    scores: { composite: 71.9, strategy: 78, tire: 80, pace: 90, pit: 82, execution: 30 },
    position: 6,
    gridPosition: 2,
    status: "Classified"
  },
  {
    driver: DRIVERS.LEC,
    scores: { composite: 76.4, strategy: 74, tire: 80, pace: 82, pit: 76, execution: 70 },
    position: 7,
    gridPosition: 7,
    status: "Classified"
  },
  {
    driver: DRIVERS.PER,
    scores: { composite: 58.2, strategy: 52, tire: 60, pace: 64, pit: 70, execution: 45 },
    position: 8,
    gridPosition: 9,
    status: "Classified"
  }
...
export const STRATEGIES = {
  ...
  NOR: [
    ...
    { compound: "medium", startLap: 50, endLap: 71, wearSlope: 0.06, isActual: true }
  ],
  RUS: [
  ...
  HAM: [
    { compound: "medium", startLap: 1, endLap: 23, wearSlope: 0.06, isActual: true },
...
export const SAI_ALTERNATIVE_STINTS = [
...
export const KEY_INSIGHTS = [
  ...
  },
  {
    ...
    metric: { value: 1.4, unit: "s", context: "Simulated gain from pit on Lap 20 vs actual Lap 22" },
    confidence: "medium",
...
export const TEAM_METRICS = [
  ...
  },
  {
    ...
    constructorScore: 79.5,
    pitCrewRank: 1,
    strategyGrade: "B+",
    avgWearSlope: 0.065
  }
];
export const DEMO_INVESTIGATION = [
  {
    id: "msg-1",
    type: "verdict",
...
export const FEATURED_STORIES = [
  {
    ...
    keyMoments: [
      { lap: 22, description: "Sainz pits early \u2014 Ferrari commits to aggressive strategy", type: "strategy" },
      { lap: 51, description: "Norris closes to within DRS range of Verstappen", type: "overtake" },
      { lap: 64, description: "Contact between VER and NOR \u2014 both sustain damage", type: "incident" }
    ],
    raceId: "aut-2024"
  }
];
export const SPIELBERG_CORNERS = [
...
function generateTelemetry(driverProfile, seed) {
  const points = [];
  const trackLength = 4318;
  for (let d = 0; d <= trackLength; d += 10) {
    ...
    } else if (d >= 1e3 && d < 1100) {
      speed = 282 - (d - 1e3) * 1.5;
      brake = Math.min(100, (d - 1e3) * 0.9);
      throttle = 0;
      gear = Math.max(3, 8 - Math.floor((d - 1e3) / 18));
    ...
    const variation = driverProfile === "aggressive" ? Math.sin(d * 0.01 + seed) * 3 : Math.sin(d * 0.01 + seed + 1) * 2.5;
    points.push({
      distanceM: d,
...
export const GHOST_BATTLE_NARRATIONS = [
  {
    cornerIndex: 0,
