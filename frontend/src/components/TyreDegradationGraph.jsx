...
export function TyreDegradationGraph({
  ...
  return <div className={cn("bg-panel border border-fw-border rounded-card p-4 flex flex-col gap-3", className)}><div className="flex justify-between items-center text-mono-meta font-mono"><span className="text-text-primary font-semibold tracking-wider flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-amber-400" />
        ...
        </span><div className="flex items-center gap-3 text-[10px]"><span className="text-text-muted">{String(driverCode || "DRIVER").toUpperCase()} COMPOUND: <span className="text-amber-400 font-bold">{latest?.compound || "MEDIUM"}</span></span><span className="text-text-muted">LIFE: <span className="text-drs-cyan font-bold">{latest?.wear_pct || 75}%</span></span></div></div><div className="relative w-full h-[160px]"><svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">{[0, 25, 50, 75, 100].map((pct, idx) => {
