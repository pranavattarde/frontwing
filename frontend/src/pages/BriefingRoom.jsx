...
export function BriefingRoom() {
  ...
  }<div className="flex-1 flex flex-col gap-6 max-w-2xl w-full"><div className="flex flex-col gap-2"><span className="text-mono-meta font-mono text-drs-cyan tracking-widest uppercase">
                ENGINEER_ROOM // ACTIVE_SESSION
              </span><h1 className="text-display text-text-primary">
                Could Ferrari have won the Austrian Grand Prix?
              </h1><p className="text-text-muted text-sm max-w-lg">
  ...
  }<div className="absolute inset-0 bg-[linear-gradient(to_right,#1c2025_1px,transparent_1px),linear-gradient(to_bottom,#1c2025_1px,transparent_1px)] bg-[size:24px_24px] opacity-10" /><div className="relative flex flex-col items-center w-full"><span className="absolute top-0 left-0 text-[10px] font-mono text-text-muted">
                CIRCUIT // RED_BULL_RING
              </span><span className="absolute top-0 right-0 text-[10px] font-mono text-text-muted">
                LEN: 4.318 KM
              </span>{
  ...
  /><path
    ...
    strokeWidth="2.5"
    strokeDasharray="4,4"
  /></svg><div className="flex justify-between w-full mt-4 border-t border-fw-border pt-4 text-mono-meta font-mono"><div><span className="text-text-muted">TURNS: </span><span className="text-text-primary">10</span></div><div><span className="text-text-muted">DRS_ZONES: </span><span className="text-text-primary">3</span></div><div><span className="text-text-muted">RECORD: </span><span className="text-text-primary">1:05.619</span></div></div></div></div></section>{
    /* Recent & Saved Investigations Section */
  }{(isLoadingHistory || recentInvestigations.length > 0 || savedInvestigations.length > 0) && <section className="flex flex-col gap-6 border-t border-fw-border pt-8"><div className="flex items-center justify-between"><span className="text-mono-meta font-mono text-drs-cyan uppercase tracking-wider">
                INVESTIGATION_HISTORY // ARCHIVE
              </span><span className="text-mono-meta font-mono text-text-muted">{isLoadingHistory ? "LOADING..." : `${recentInvestigations.length} TOTAL DEBRIEFS`}</span></div>{isLoadingHistory ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{[1, 2, 3].map((i) => <div key={i} className="h-28 rounded-card border border-fw-border/60 bg-panel/20 animate-pulse p-4 flex flex-col justify-between"><div className="h-3 w-1/3 bg-fw-border/40 rounded" /><div className="h-4 w-3/4 bg-fw-border/60 rounded" /><div className="h-3 w-1/2 bg-fw-border/30 rounded" /></div>)}</div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{recentInvestigations.map((item) => <div
    key={item.id}
  ...
  ><div className="flex flex-col gap-2"><div className="flex items-center justify-between text-[10px] font-mono text-text-muted"><span className="text-drs-cyan/80 uppercase">{item.session || "SPIELBERG_GP"}</span><span>{new Date(item.timestamp || item.created_at).toLocaleDateString()}</span></div><h4 className="text-sm font-sans font-medium text-text-primary group-hover:text-drs-cyan transition-colors line-clamp-2">{item.question}</h4></div><div className="flex items-center justify-between border-t border-fw-border/60 pt-3 text-[10px] font-mono"><span className="text-text-muted">
                      PROVIDER: <span className="text-text-primary uppercase">{item.provider_used || "GEMINI"}</span></span><button
  ...
  >
    ...
    /* Featured Race Story */
  }<div className="xl:col-span-2 flex flex-col gap-4"><div className="flex items-center justify-between"><span className="text-mono-meta font-mono text-text-muted uppercase tracking-wider">
                FEATURED_DEBRIEF
              </span><button
  ...
  >
              ...
              </button></div>{FEATURED_STORIES.map((story) => <RaceStoryCard
    key={story.id}
    title={story.title}
  ...
  }<div className="flex flex-col gap-4"><span className="text-mono-meta font-mono text-text-muted uppercase tracking-wider">
              TRENDING_TACTICAL_INSIGHTS
            </span><div className="flex flex-col sm:flex-row xl:flex-col gap-3">{KEY_INSIGHTS.slice(0, 3).map((insight, idx) => <InsightCard
    key={idx}
    insight={insight}
    variant="featured"
  ...
  />)}</div></div></section></main>{
    /* Footer Info */
  }<footer className="border-t border-fw-border py-4 bg-panel/30 mt-auto"><div className="max-w-[1440px] mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-mono-meta font-mono text-text-muted"><div>
            SYSTEM_STATUS: <span className="text-drs-cyan">ACTIVE</span> // FASTF1_SAMPLING: <span className="text-text-primary">10M_BINS</span></div><div>
