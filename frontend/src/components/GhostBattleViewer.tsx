...
import type { CornerNarration, TelemetryPoint } from '@/lib/types';

interface GhostBattleViewerProps {
  driverA: { code: string; color: string; data: TelemetryPoint[] };
  driverB: { code: string; color: string; data: TelemetryPoint[] };
  lapNumber: number;
  trackName: string;
  narrations: CornerNarration[];
  className?: string;
}
...
export function GhostBattleViewer({
  ...
  narrations,
  className,
}: GhostBattleViewerProps) {
  const [activeCornerIdx, setActiveCornerIdx] = useState<number>(0);
  const [hoverDist, setHoverDist] = useState<number | null>(null);
  const [viewMode, setViewMode] = useState<'single' | 'stacked'>('single');

  // Convert Corner Narrations to Telemetry Annotations
  ...
  // Set highlight zone on telemetry based on selected corner
  const highlightZone = useMemo(() => {
    const activeDistance = annotations[activeCornerIdx]?.distanceM;
    if (!activeDistance) return undefined;
    return {
  ...
  const handleHover = (dist: number) => {
    setHoverDist(dist);
    // Find closest corner to highlight in narration
    const closestIdx = annotations.reduce(
      (closest, curr, idx) =>
        Math.abs(curr.distanceM - dist) < Math.abs(annotations[closest].distanceM - dist) ? idx : closest,
      0
    );
  ...
  return (
    <div className={cn('grid grid-cols-1 lg:grid-cols-12 gap-8 items-start select-none', className)}>
            ...
            <button
              ...
              className={`px-1.5 py-0.5 border rounded-sm ${
                viewMode === 'single'
                  ...
                  : 'border-fw-border text-text-muted'
              }`}
            >
              SINGLE_TRACE
            </button>
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  isActive
                    ? 'border-fw-border-active bg-elevated/40'
                    : 'border-fw-border bg-panel hover:bg-elevated/20'
                )}
              ...
              >
                ...
                <div className="pl-2">
                  <div className="flex justify-between items-baseline mb-2 font-mono text-mono-meta">
                    <span className="text-text-primary font-semibold">{nar.cornerName}</span>
                    <span className={cn('text-xs font-semibold', isAdvantageA ? 'text-drs-cyan' : 'text-teammate-yellow')}>
                      {isAdvantageA ? driverA.code : driverB.code} ADV (+{Math.abs(nar.deltaMs)}ms)
                    </span>
                  </div>
