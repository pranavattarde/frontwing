...
export function RaceTimeline({
  ...
  if (isVertical) {
    return <div className="flex flex-col gap-4 relative before:absolute before:left-[15px] before:top-2 before:bottom-2 before:w-[2px] before:bg-fw-border">{phases.map((phase, idx) => {
    ...
    }{incidents.length > 0 && <div className="mt-4 pt-4 border-t border-fw-border flex flex-col gap-2"><span className="text-mono-meta font-mono text-text-muted uppercase tracking-wider">
    ...
    ><div className="flex justify-between font-mono text-[9px] text-f1-red mb-1"><span>⚠ INCIDENT_REPORTER</span><span>LAP {inc.lap}</span></div><p className="text-text-secondary">{inc.description}</p><div className="flex gap-2 mt-2">{inc.drivers.map((d) => <span key={d} className="font-mono text-[9px] bg-f1-red/15 text-f1-red px-1 border border-f1-red/25 rounded-sm">{d}</span>)}</div></div>)}</div></div>}</div>;
  }
  return <div className="w-full bg-panel border border-fw-border rounded-card p-3 select-none overflow-x-auto"><div className="min-w-[760px] flex items-center justify-between gap-1 relative before:absolute before:left-2 before:right-2 before:h-0.5 before:bg-fw-border">{phases.map((phase, idx) => {
    const isActive = activePhaseIndex === idx;
    return <div
      key={idx}
      onClick={() => onPhaseClick?.(idx)}
    ...
    /><span className="font-mono text-[9px] mt-1 font-semibold">L{phase.startLap}-{phase.endLap}</span><span className="text-[10px] text-center truncate max-w-[120px] text-text-secondary mt-1 group-hover:text-text-primary transition-colors">{phase.description}</span></div>;
  })}</div></div>;
}
