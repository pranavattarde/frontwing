...
export const BriefingHeader = memo(function BriefingHeader2({
  ...
  ><span className="text-drs-cyan font-mono text-mono-meta font-semibold tracking-widest group-hover:text-text-primary transition-colors duration-[80ms]">
              FW
            </span><span className="hidden sm:inline text-text-muted font-mono text-mono-meta tracking-wider group-hover:text-text-secondary transition-colors duration-[80ms]">
              FRONTWING
            </span></button>{
    /* Separator */
  }{breadcrumbs.length > 0 && <span className="text-text-muted mx-1">/</span>}{
    /* Breadcrumbs */
  }<div className="flex items-center gap-1 min-w-0 overflow-hidden"><AnimatePresence mode="popLayout">{breadcrumbs.map((crumb, index) => {
    ...
    >{index > 0 && <span className="text-text-muted text-xs">/</span>}<button
      onClick={() => onBreadcrumbClick?.(index)}
      className={cn(
        "text-mono-meta font-mono truncate max-w-[160px] transition-colors duration-[80ms]",
        isLast ? "text-text-primary" : "text-text-muted hover:text-text-secondary hover:underline underline-offset-2"
      )}
      aria-current={isLast ? "page" : void 0}
    >{crumb.label}</button></motion.div>;
  })}</AnimatePresence></div>{
    /* Session State Indicator */
  }{sessionState !== "idle" && <div className="flex items-center gap-1.5 ml-2" aria-live="polite"><div
    className={cn(
  ...
  ><svg
    ...
    strokeWidth={2}
    stroke="currentColor"
  ><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg><span className="hidden sm:inline text-mono-meta font-mono text-text-muted group-hover:text-text-secondary">
            ⌘K
          </span></button></div></nav>;
});
