...
import type { Driver, DriverScores } from '@/lib/types';
      ...
      <div className="pl-2">
        ...
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex flex-col items-end">
            <span className="font-data text-sm font-semibold text-drs-cyan">
              P{position}
            </span>
            <span className={cn(
              ...
              {posChange > 0 ? `▲${posChange}` : posChange < 0 ? `▼${Math.abs(posChange)}` : 'static'}
            </span>
          </div>
        </div>
        ...
        {isDetailed ? (
          <div className="flex flex-col gap-4">
            {/* Detailed Stats Grid */}
            <div className="grid grid-cols-2 gap-3 border-t border-b border-fw-border py-3">
              <div className="flex flex-col">
                <span className="text-[9px] font-mono text-text-muted">TEAM</span>
                <span className="text-xs font-medium text-text-secondary truncate">{driver.teamName}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-mono text-text-muted">STATUS</span>
                <span className="text-xs font-mono text-text-secondary">{status}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-mono text-text-muted">GRID_START</span>
                <span className="text-xs font-mono text-text-secondary">P{gridPosition}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-mono text-text-muted">GRID_DIFF</span>
                <span className={cn(
                  'text-xs font-mono',
            ...
            <div className="flex items-center justify-between">
              <ScoreRing value={scores.composite} label="COMPOSITE" size="lg" color="#00E5FF" />
                ...
                {Object.entries(scores)
                  ...
                  .map(([key, val]) => (
                    <div key={key} className="flex justify-between items-center text-mono-meta font-mono">
                      <span className="text-text-muted uppercase text-[9px]">{key}</span>
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-elevated rounded-sm overflow-hidden border border-fw-border">
                          <div
                            className="h-full bg-drs-cyan"
                            style={{ width: `${val}%` }}
                          />
                        </div>
                        <span className="text-text-primary text-[10px] w-6 text-right">{val}</span>
                      </div>
                    </div>
                  ))}
              </div>
          ...
          /* Compact View */
          <div className="flex items-center justify-between border-t border-fw-border pt-2.5">
            <div className="flex flex-col">
              <span className="text-[9px] font-mono text-text-muted uppercase">Constructor</span>
              <span className="text-xs text-text-secondary font-medium truncate max-w-[120px]">
                {driver.teamName}
              </span>
            </div>
            <ScoreRing value={scores.composite} label="SCORE" size="md" color="#00E5FF" />
          </div>
        )}
      </div>
