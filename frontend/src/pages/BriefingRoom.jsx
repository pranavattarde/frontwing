  ...
  return (
    <div className="min-h-screen bg-canvas text-text-secondary flex flex-col font-sans selection:bg-drs-cyan/20 selection:text-drs-cyan">
      ...
      <main className="flex-1 w-full max-w-[1440px] mx-auto px-4 py-8 flex flex-col justify-between gap-12">
        ...
        <section className="flex flex-col lg:flex-row items-center justify-between gap-12 my-auto">
          ...
          <div className="flex-1 flex flex-col gap-6 max-w-2xl w-full">
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                ...
                {heroData?.last_updated && (
                  <span className="text-[10px] font-mono text-text-muted/60 px-1.5 py-0.5 rounded border border-fw-border/40">
            ...
            {heroData?.sessions && heroData.sessions.length > 0 && (
              <div className="flex flex-col gap-2 p-3.5 rounded-card border border-fw-border/60 bg-panel/30">
                <div className="flex items-center justify-between text-mono-meta font-mono text-text-muted">
                  <span className="text-drs-cyan uppercase tracking-wider flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-drs-cyan animate-pulse" />
                    FASTF1 EVENT SESSIONS // IST (UTC+5:30) & LOCAL TIME
                  </span>
                  <span>ROUND {heroData.round_number || 14} • {heroData.season || 2026}</span>
                </div>
                    ...
                    <div
                      ...
                      className={`flex flex-col p-2 rounded border text-left transition-colors ${
                        sess.name.toLowerCase().includes("race")
                          ? "bg-drs-cyan/5 border-drs-cyan/40"
                          : "bg-surface/40 border-fw-border/50 hover:border-fw-border"
                      }`}
                    >
                      <span className="text-[11px] font-sans font-semibold text-text-primary truncate">
                        {sess.name}
                      </span>
                      <span className="text-[10px] font-mono text-drs-cyan font-medium mt-0.5">
                        {sess.ist}
                      </span>
          ...
          <div className="w-full lg:w-[440px] flex items-center justify-center shrink-0 border border-fw-border rounded-card p-6 bg-panel/30 relative overflow-hidden group">
            ...
            <div className="relative flex flex-col items-center w-full">
              <span className="absolute top-0 left-0 text-[10px] font-mono text-text-muted uppercase">
              ...
              <div className="w-full h-[240px] flex items-center justify-center pt-5">
                <svg
                  ...
                  className="w-full h-full text-text-muted/50 group-hover:text-drs-cyan/60 transition-colors duration-300"
                  stroke="currentColor"
                  strokeWidth="1.75"
                  fill="none"
                  strokeLinecap="round"
                ...
                >
                  ...
                  {circuit.startFinish && (
                    <circle
                      ...
                      r="4"
                      className="fill-drs-cyan text-drs-cyan"
                    />
                  )}
                </svg>
              </div>
              ...
              <div className="flex justify-between w-full mt-4 border-t border-fw-border pt-4 text-mono-meta font-mono">
                <div>
                  ...
                  <span className="text-text-primary">{heroData?.turns || 14}</span>
                </div>
                <div>
                  <span className="text-text-muted">DRS_ZONES: </span>
                  <span className="text-text-primary">{heroData?.drs_zones || 2}</span>
                </div>
                <div>
                  <span className="text-text-muted">RECORD: </span>
                  <span className="text-text-primary">{heroData?.lap_record || "1:16.330"}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Real User Investigation History (Fix U) */}
          ...
          {isLoadingHistory ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-28 rounded-card border border-fw-border/60 bg-panel/20 animate-pulse p-4 flex flex-col justify-between"
                >
            ...
            <div className="border border-dashed border-fw-border/70 rounded-card p-6 flex flex-col sm:flex-row items-center justify-between gap-4 bg-panel/20">
              <div className="flex flex-col gap-1 text-left">
                <span className="text-xs font-mono text-drs-cyan uppercase tracking-wider">
              ...
              <button
                onClick={openAuth}
                className="px-4 py-2 text-xs font-mono font-semibold text-canvas bg-drs-cyan hover:bg-drs-cyan/90 rounded-sm uppercase tracking-wider transition-colors shrink-0"
              >
                SIGN IN / REGISTER
              </button>
            </div>
            ...
            <div className="border border-dashed border-fw-border rounded-card p-8 flex flex-col items-center justify-center text-center gap-2 bg-panel/10">
        ...
        {/* Real Live Editorial Pipeline (Fix V) */}
        <section className="grid grid-cols-1 xl:grid-cols-3 gap-8 border-t border-fw-border pt-8">
          ...
          <div className="xl:col-span-2 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-mono-meta font-mono text-drs-cyan">
            ...
            {featuredStories.length > 0 ? (
              ...
              <div className="p-8 border border-fw-border rounded-card bg-panel/20 text-center">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row xl:flex-col gap-3">
              {trendingInsights.length > 0 ? (
                ...
                <div className="p-6 border border-fw-border rounded-card bg-panel/20 text-center">
      ...
      <footer className="border-t border-fw-border py-4 bg-panel/30 mt-auto">
        <div className="max-w-[1440px] mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-mono-meta font-mono text-text-muted">
          <div>
            SYSTEM_STATUS: <span className="text-drs-cyan">ACTIVE</span> // FASTF1_INTEGRATION:{" "}
            <span className="text-text-primary uppercase">{heroData?.event_name || "ONLINE"}</span>
          </div>
          <div>
