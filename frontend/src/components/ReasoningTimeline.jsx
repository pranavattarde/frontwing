...
export function ReasoningTimeline({ events, totalDuration, className }) {
  return <div className={cn("bg-surface-base border border-border-subtle rounded-card p-4 flex flex-col gap-4 select-none w-full", className)}>{
    /* Title */
  }<div className="flex justify-between items-center text-mono-meta font-mono border-b border-border-subtle pb-2"><span className="text-text-primary font-semibold tracking-wide">
          Reasoning Pipeline Latency
        </span><span className="text-text-muted type-tabular">Total Duration: {totalDuration}ms</span></div>{
    /* Latency ticks block */
    ...
    /><div className="flex-1 flex justify-between items-baseline gap-2 font-mono"><div className="flex flex-col gap-0.5"><span className="text-xs text-text-primary">
                    +{evt.timestamp}ms: {evt.description}</span><span className="text-[9px] text-text-muted capitalize">
                    Type: {evt.type?.replace(/_/g, " ")}</span></div><span className="text-text-muted text-[10px] shrink-0 font-semibold">{evt.duration_ms}ms
                </span></div></motion.div>;
  })}</div></div>;
}
