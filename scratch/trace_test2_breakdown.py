...
def trace_test2():
    ...
    for k in range(1, total_laps + 1):
        ...
        is_actual_pit = driver_actual_laps[k-1].get("is_pit_out_lap", False)
        if is_actual_pit:
            actual_pit_laps.append(k)
        ...
        if is_pit:
            note = f"SIM PIT STOP ({pit_loss:.1f}s loss) + {note}"
        if is_actual_pit:
            note = f"[ACTUAL PIT OUT LAP] + {note}"
    ...
    print(f"ACTUAL PIT OUT LAPS IN DB: {actual_pit_laps}")
