...
export function DriverCard({
  ...
  /><div className="pl-2">{
    /* Top Header */
  }<div className="flex justify-between items-start mb-3"><div><div className="flex items-center gap-1.5"><span className="font-data text-base font-semibold text-text-primary">{driver.code}</span><span className="text-[10px] font-mono text-text-muted type-tabular">
                #{driver.number}</span></div><p className="text-xs text-text-muted leading-none mt-0.5">{driver.fullName}</p></div>{
    /* Race Result Badge */
  }<div className="flex flex-col items-end"><span className="font-data text-sm font-semibold text-timing-green type-tabular">
