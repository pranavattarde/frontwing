...
interface CommandItem {
  id: string;
  label: string;
  category: 'NAVIGATION' | 'ACTIONS' | 'QUERIES';
  shortcut?: string;
  action?: () => void;
}
...
export function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  useEffect(() => {
    const handleGlobalKey = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        e.preventDefault();
        // Custom event to trigger open state in parent
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[300] flex items-start justify-center p-4 pt-[15vh]">
          {/* Backdrop */}
          <motion.div
          ...
          >
            {/* Input row */}
            <div className="flex items-center px-4 py-3 border-b border-fw-border bg-canvas/20">
            ...
            {/* List */}
            <div className="max-h-[300px] overflow-y-auto divide-y divide-fw-border/20 p-2">
              {filtered.length > 0 ? (
