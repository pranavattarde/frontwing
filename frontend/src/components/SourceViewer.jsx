...
export function SourceViewer({
  ...
  >{showRaw ? "[VIEW_SUMMARY]" : "[INSPECT_RAW_JSON]"}</button></div>{
  ...
  }{!showRaw ? <div className="flex flex-col gap-2"><p className="text-xs text-text-secondary leading-relaxed">{formattedExplanation}</p><div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-fw-border/60 font-mono text-[9px] text-text-muted"><div>
    ...
    <div className="bg-canvas border border-fw-border rounded-card p-3 max-h-[160px] overflow-y-auto"><pre className="font-mono text-[10px] text-text-secondary leading-normal overflow-x-auto whitespace-pre-wrap select-all">{JSON.stringify(rawData, null, 2)}</pre></div>
  )}</div>;
}
