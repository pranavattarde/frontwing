...
class TestProductionIntegration(unittest.TestCase):
    ...
    @patch("app.core.providers.HAS_GEMINI", True)
    def test_reliable_llm_planning_direct_execution(self):
        """Verifies that planning requests invoke the LLM live without artificial plan caching."""
        mock_gemini = MagicMock()
        mock_gemini.models.generate_content.return_value = MagicMock(
            text='{"intent": "direct_test", "complexity": "low", "required_engineers": [], "required_tools": [], "execution_order": [], "expected_evidence": [], "fallback_plan": []}'
        )
        
        os.environ["GEMINI_API_KEY"] = "mock_key"
        with patch("app.core.providers.genai.Client", return_value=mock_gemini):
            # First call: should hit the client
            plan1, metrics1 = reliable_llm_provider.generate_plan("System instructions fresh", "User query fresh")
            self.assertEqual(plan1["intent"], "direct_test")
            self.assertEqual(mock_gemini.models.generate_content.call_count, 1)
            
            # Second call: executes live again (no stale in-memory plan cache)
            plan2, metrics2 = reliable_llm_provider.generate_plan("System instructions fresh", "User query fresh")
            self.assertEqual(plan2["intent"], "direct_test")
