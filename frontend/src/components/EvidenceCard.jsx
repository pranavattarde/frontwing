...
export function EvidenceCard({
  ...
  >{
    /* Card Header */
  }<div className="flex justify-between items-start border-b border-fw-border pb-2.5 mb-3"><div><h3 className="text-text-primary text-sm font-semibold tracking-tight">{title}</h3>{subtitle && <p className="text-mono-meta font-mono text-text-muted mt-0.5 uppercase">{subtitle}</p>}</div>{
    /* Action Controls */
  }<div className="flex items-center gap-3">{onExport && <button
    onClick={onExport}
    className="text-mono-meta font-mono text-text-muted hover:text-text-secondary transition-colors"
    title="Export"
  >
              [EXPORT]
            </button>}{onDeepDive && !isCollapsed && <button
    onClick={onDeepDive}
  ...
  ><svg
    ...
    strokeWidth={2}
    stroke="currentColor"
  ><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg></button>}</div></div>{
    /* Visual Content Area */
  ...
  }{variant === "collapsed" && <div className="absolute top-0 left-0 right-0 h-0.5 bg-drs-cyan/10 overflow-hidden rounded-t-card"><div className="h-full bg-drs-cyan/40 w-1/3 animate-skeleton" /></div>}</motion.section>;
}
