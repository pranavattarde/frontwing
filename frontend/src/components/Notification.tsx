...
/** component_library.md §17: Notification (toast variant) */
function NotificationToast({ notification, onDismiss }: NotificationToastProps) {
  ...
  return (
    ...
    >
      <span className={cn('font-mono text-sm', COLORS[notification.type].split(' ')[0])}>
...
/** Notification container — max 3 toasts, bottom-right */
export function NotificationContainer() {
  ...
  useEffect(() => {
    (window as unknown as Record<string, unknown>).__fw_notify = (n: NotificationData) => {
    ...
    return () => {
      delete (window as unknown as Record<string, unknown>).__fw_notify;
    };
  }, []);

  return (
    <div className="fixed bottom-4 right-4 z-[400] flex flex-col gap-2 w-[360px] max-w-[calc(100vw-32px)]">
